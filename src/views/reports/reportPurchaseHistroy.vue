...
const getPurchaseAsset = async () => {
    ...
    try {
        ...
        if (selectedAsset.value) {
            dynamicConditions.push({ field: "assets.assetId", operator: "==", value: selectedAsset.value });
        }
        ...
        // Flatten assets array so each asset is a row
        const flat = [];
        (response.data.data || []).forEach(item => {
            (item.assets || []).forEach(asset => {
                flat.push({
                    ...item,
                    asset, // single asset object
                });
            });
        });
        if (selectedAsset.value) {
            tableData.value = flat.filter(item => item.asset.assetId === selectedAsset.value);
        } else {
...
const exportToExcel = () => {
    const ws_data = [
        ['ID', 'Created By', 'Asset Name', 'Quantity', 'Price', 'Total Price', 'Created At', 'Description'],
        ...tableData.value.map((item, index) => [
            index + 1,
            getUserName(item.createdBy),
            item.asset?.assetName || 'Unknown',
            item.asset?.qty || 0,
            item.asset?.price
                ? `${new Intl.NumberFormat('en-US').format(item.asset.price)} ${item.asset.currency?.symbol.symbol || ''}`
                : `0 ${item.asset.currency?.symbol.symbol || ''}`,
            item.asset?.totalPrice
                ? `${new Intl.NumberFormat('en-US').format(item.asset.totalPrice)} ${item.asset.currency?.symbol.symbol || ''}`
                : `0 ${item.asset.currency?.symbol.symbol || ''}`,
    ...
    XLSX.writeFile(wb, 'Report_PurchaseAsset.xlsx');
};
...
<template>
    <div class="p-5 w-full bg-white shadow-md rounded-md font-khmer hidden md:block">
        <p class="text-left font-semibold text-lg">{{$t('Report purchase assets history')}}</p>
    ...
    <div class="font-khmer p-5 w-full shadow-md rounded-md bg-white mt-5 hidden md:block">
        <div ref="printableDiv" class="print-area overflow-x-auto mt-3 relative">
            <div class="text-sm font-semibold mb-4">
                ...
                <h2 class="text-center">បញ្ជីរបាយការណ៍ការគ្រប់គ្រងការចំណាយសម្ភារៈរបស់ក្រុមហ៊ុន</h2>
            ...
            <table ref="tableRef" class="w-full mt-3 border-collapse" :class="{ 'opacity-50': isLoading == true }">
                <thead class="bg-gray-100 text-xs">
                    <tr class="border border-gray-400">
                        ...
                        <th class="border border-gray-300 p-2">{{$t('PurchaseBy')}}</th>
                        <th class="border border-gray-300 p-2">{{$t('Asset')}}</th>
                        <th class="border border-gray-300 p-2">{{$t('Quantity')}}</th>
                        <th class="border border-gray-300 p-2">{{$t('Price')}}</th>
                        <th class="border border-gray-300 p-2">{{$t('Total Price')}}</th>
                        <th class="border border-gray-300 p-2">{{$t('CreateAt')}}</th>
                        <th class="border border-gray-300 p-2">{{$t('Descriptions')}}</th>
                    </tr>
                </thead>
                <tbody class="text-sm" v-if="tableData.length > 0">
                    <tr v-for="(item, index) in tableData" :key="index">
                        ...
                        <td class="border border-gray-300 p-2">{{ item.asset?.assetName || 'Unknown' }}</td>
                        <td class="border border-gray-300 p-2">{{ item.asset?.qty || 0 }}</td>
                        <td class="border border-gray-300 p-2">
                            {{ item.asset?.price ? new Intl.NumberFormat('en-US').format(item.asset.price) : 0 }}
                            {{ item.asset.currency?.symbol.symbol }}
                        </td>
                        <td class="border border-gray-300 p-2">
                            {{ item.asset?.totalPrice ? new Intl.NumberFormat('en-US').format(item.asset.totalPrice) : 0
                            }}
                            {{ item.asset.currency?.symbol.symbol }}
                        </td>
                ...
                <tbody v-else>
                    <tr class="[&>*]:border [&>*]:p-3 text-gray-500">
                        <td colspan="12" class="font-khmer text-center text-lg">{{$t('No Data!')}}</td>
                    </tr>
                </tbody>
            </table>
        </div>
