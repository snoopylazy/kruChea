    ...
    try {
        ...
        const params = {
            dynamicConditions: JSON.stringify([
                { field: '_id', operator: '==', value: props.id }
            ]),
        ...
        if (transactions.length > 0) {
            ...
            console.error("No transaction found with ID:", props.id);
            errorMessage.value = t('Transaction not found');
            showError.value = true;
        }
    } catch (error) {
        console.error("Error fetching transaction:", error);
        errorMessage.value = t('Failed to load transaction details');
        showError.value = true;
    } finally {
...
// Enhanced function to create cutting stock records for checkout operations
...
// Add new function to handle returns
...
const handleSubmit = async () => {
    try {
        if (!approvedDate.value) {
            errorMessage.value = t('Please select an approval date');
            showErrorMessage.value = true;
            errorAudio.play();
        ...
        if (!selectType.value) {
            errorMessage.value = t('Please select approve or reject');
            showErrorMessage.value = true;
            errorAudio.play();
            setTimeout(() => (showError.value = false), 3000);
            return;
        ...
        if (!transactionData.value) {
            errorMessage.value = t('Transaction data is missing');
            showErrorMessage.value = true;
            errorAudio.play();
            setTimeout(() => (showError.value = false), 3000);
            return;
        ...
        if (
            ...
            (selectType.value.value === 'rejected' && !canRejectTransaction())
        ) {
            errorMessage.value = t('You do not have permission to perform this action.');
            showErrorMessage.value = true;
            errorAudio.play();
            setTimeout(() => (showError.value = false), 3000);
            return;
        ...
        if (selectType.value.value === 'approved' && !isAlreadyApproved) {
            ...
            if (transactionData.value.transactionType === 'checkout') {
                ...
                // First create cutting stock entries for checkout
                ...
                // Then update the asset's total stock
                ...
                // Create return cutting stock entries
                ...
                // Update the return transaction with the actual return date
                updatedFields.fields.actualReturnDate = timestamp;
            }
        }

        successMessage.value = t('Status updated successfully!');
        showSuccessMessage.value = true;
        successAudio.play();
        setTimeout(() => {
            showSuccessMessage.value = false;
        ...
        errorMessage.value = t('Failed to process your request. Please try again.');
        showErrorMessage.value = true;
        errorAudio.play();
        setTimeout(() => (showError.value = false), 3000);
    } finally {
...
<template>
    <div>
        ...
        <router-link to="/operation-assets"
            ...
            <span>{{ $t('Approved Form') }}</span>
        </router-link>


        <div class="mt-3 space-y-6 px-4 py-6 bg-white rounded-2xl shadow-lg  sm:max-w-xl mx-auto">
            ...
            <div v-if="isLoading" class="py-8 flex flex-col items-center justify-center">
                ...
                <p class="text-sm text-gray-600">{{ $t('Loading transaction details...') }}</p>
            </div>
                ...
                <form @submit.prevent="handleSubmit" class="space-y-6">
                    ...
                    <div class="text-start">
                        <label for="approvedDate" class="text-sm font-medium text-gray-700 block mb-1">
                            {{ $t('Approved Date') }} <span class="text-red-500">*</span>
                        </label>
                    ...
                    <div class="text-start">
                        ...
                        <Select v-model="selectType" :options="filteredTypes" optionLabel="name"
                            :placeholder="$t('Select Type')" class="w-full" :disabled="loading" />
                    </div>
