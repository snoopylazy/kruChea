...
const createStockTransfer = async (transferData) => {
  try {
    ...
    errorMessage.value = error?.response?.data?.message || error.message || t('Failed to create stock transfer');
...
const handleSubmit = async () => {
  try {
    ...
    formErrors.submit = null;
    formErrors.assetErrors = [];

    // Validate form
    if (!selectedToBranches.value?.length) {
      formErrors.toBranch = t('Please select at least one destination branch');
      return;
    }

    if (!assets.value.some(a => a.selectedAsset && a.quantity > 0)) {
      formErrors.assets = t('Please select at least one asset with quantity');
      return;
    }

    // Check if any selected branch is the same as current branch
    if (selectedToBranches.value.includes(branchStore.branchId)) {
      formErrors.toBranch = t('Cannot transfer to the same branch');
      return;
    }

    let lastResult;
    // Loop through all selected branches
    for (const selectedBranchId of selectedToBranches.value) {
      const selectedBranch = toBranches.value.find(b => b._id === selectedBranchId);
      if (!selectedBranch) continue;

      // Prepare transfer data for each branch
      const transferData = {
        fromBranchId: branchStore.branchId,
        toBranchId: selectedBranchId,
        toBranchPrefix: selectedBranch.abbreviation,
      ...
      lastResult = await createStockTransfer(transferData);
    }
    ...
    formErrors.submit = error.message || t('Failed to create stock transfer');
  } finally {
...
const clearForm = () => {
  ...
  remark.value = '';
  formErrors.submit = null;
  formErrors.toBranch = null;
  formErrors.assets = null;
  formErrors.assetErrors = [];
...
<template>
  <div>
    ...
    <form @submit.prevent="handleSubmit"
      class="mt-4 space-y-5 px-4 py-6 text-start bg-white rounded-lg shadow-md border border-gray-200">
      <h3 class="text-lg font-semibold text-gray-800 text-start">{{ $t('Create Stock Transfer') }}</h3>

      <!-- Branch Selection -->
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <!-- From Branch -->
        <div>
          <label class="block text-xs text-gray-700 text-left mb-1">{{ $t('From') }} <span
        ...
        <div v-if="isLoading" class="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div class="bg-white p-5 rounded-lg shadow-lg flex flex-col items-center">
            <div class="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-emerald-500 mb-3">
            </div>
            <p>{{ $t('Processing...') }}</p>
          </div>
        </div>
        ...
        <!-- To Branch -->
        <div>
          <label class="block text-xs text-gray-700 text-left mb-1">{{ $t('To') }} <span
              class="text-red-500">*</span></label>
          <MultiSelect v-model="selectedToBranches" :options="toBranches" optionLabel="abbreviation" optionValue="_id"
            display="chip" :placeholder="$t('Select Branch')" class="w-full" filter />
      ...
      <div class="space-y-4">
        <div class="flex justify-between items-center">
          <h3 class="text-sm font-medium">{{ $t('Assets') }} <span class="text-red-500">*</span></h3>
          <button type="button" @click="addAsset"
            class="flex items-center bg-gradient-to-br from-emerald-400 to-emerald-700 text-white p-2 rounded-md text-xs hover:from-emerald-500 hover:to-emerald-600 transition-all duration-300 shadow-md">
            <i class="fa-solid fa-square-plus mr-1"></i> {{ $t('More') }}
          </button>
        </div>
          ...
          <div v-for="(assetItem, index) in assets" :key="index"
            ...
            <div class="flex flex-col md:flex-row gap-4">
              ...
              <div class="w-full">
                ...
                <div v-if="selectedToBranches.length > 1 && assetItem.selectedAsset" class="text-xs text-gray-500 mt-1">
                  <div>{{ $t('Total needed') }}: {{ assetItem.quantity * selectedToBranches.length }}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="flex justify-end gap-3 pt-2">
        <button type="button" @click="clearForm"
          class="bg-gradient-to-br from-red-400 to-red-600 text-white px-4 py-2 rounded-md text-sm hover:from-red-500 hover:to-red-500 transition transform hover:scale-105">
          {{ $t('Clear') }}
        </button>
        <button type="submit" :disabled="isLoading"
          class="bg-gradient-to-br from-emerald-400 to-emerald-600 text-white px-4 py-2 rounded-md text-sm hover:from-emerald-500 hover:to-emerald-500 transition transform hover:scale-105">
          {{ $t('Create') }}
        </button>
      </div>
    </form>
